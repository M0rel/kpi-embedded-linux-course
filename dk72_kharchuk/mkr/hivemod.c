...
static inline struct hive_item *hive_tree_new(unsigned long buffer_size)
{
	// (!) here's where kernel memory (probably containing secrets) leaks
        ...
        char *buf = NULL;
	buf = kzalloc(sizeof(*buf) * buffsize, GFP_KERNEL);
	if (NULL == buf)
		return NULL;
...
static int cdev_open(struct inode *inode, struct file *file)
{
	...
	if(!hive_tree_add_item(&hive_tree, item)) {
		...
		MOD_DEBUG(KERN_ERR, "Can't create file entry");
	}
	return 0;
}
			...
			unsigned long ioctl_param)
{
	...
	case UPD_BUFFER:
		if(ioctl_param > item->length) {
			char *buf = kzalloc(sizeof(*buf)*ioctl_param, GFP_KERNEL);
			memcpy(buf, item->buffer, sizeof(*buf)*item->length);
			kfree(item->buffer);
			item->buffer = buf;
...
static void module_cleanup(void)
{
	...
	for(node = rb_first(&hive_tree); node; node = rb_next(node)) {
